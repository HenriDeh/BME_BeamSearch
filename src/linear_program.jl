...
function BMEP_MILP(D::Matrix; relax = true, semi_relax = false, complete = true, init_tau = fill(2, size(D))) # D is a distance matrix (n x n)
    n = size(D,1)
    TAXA = 1:n
    taxon1 = first(TAXA)
    TAXA2n = TAXA[2:end]
...
function set_distances!(model, D)
    n = size(D,1)
    TAXA = 1:n
    LENGTHS = 2:n-1
    x = model[:x]
