...
check_radius = false
fastme_local_search(path, tree_path)
LNS_matheuristic(path, 30, tree_path, max_time = 60*60, max_it = Inf, fastme_it = Inf, inittree = false, nj_criterion = false, repair_iterate = 1, exact = true, relax = true, nj_repair = false, check_radius = check_radius)
